...
def store_cumulative_rewards(all_rewards,all_actions,seed,new_ideal_policy1,new_ideal_policy2,ideal_policy):
    ...
    filename = 'outputs/QC/QC_with_cost_gradual_finetuning_train_data_all_rewards_{}.npy'.format(seed)
    np.save(filename,all_rewards)
    filename = 'outputs/QC/QC_with_cost_gradual_finetuning_train_data_all_actions_{}.npy'.format(seed)
    np.save(filename,all_actions) 
   
    
    filename = 'outputs/QC/QC_with_cost_gradual_finetuning_train_data_new_ideal_policy1_{}.npy'.format(seed)
    np.save(filename,new_ideal_policy1)
    
    filename = 'outputs/QC/QC_with_cost_gradual_finetuning_train_data_new_ideal_policy2_{}.npy'.format(seed)
    np.save(filename,new_ideal_policy2)
    
    filename = 'outputs/QC/QC_with_cost_gradual_finetuning_train_data_ideal_policy_{}.npy'.format(seed)
