...
cost[0] = 1
cost[1] = 1
cost[2] = 2 # cost of oracle arm is twice the other arms
print(cost)

# In[ ]:


# lst_seeds = [i for i in range(10)]  # averagin over 10 runs
lst_seeds = [9]
print(lst_seeds)
...
def set_rewards(eval_examples, ideal_policy,nArms):
    ...
    print(y.shape)
    rewards_small_question = np.array(ideal_policy==0)
    rewards_big_question = np.array(ideal_policy==1)
    rewards_oracle = [1 for _ in range(len(eval_examples))]
    y[:,0] = rewards_small_question
    y[:,1] = rewards_big_question
    y[:,2] = rewards_oracle     
    
    for i in range(len(eval_examples)):
        if i<1000:
            y[i,0] = rewards_small_question[i] *0
            y[i,1] = rewards_big_question[i] *0
        elif i<2000:
            y[i,0] = rewards_small_question[i] *0.1
            y[i,1] = rewards_big_question[i] *0.1
        elif i<3000:
            y[i,0] = rewards_small_question[i] *0.2
            y[i,1] = rewards_big_question[i] *0.2
        elif i<4000:
            y[i,0] = rewards_small_question[i] *0.3
            y[i,1] = rewards_big_question[i] *0.3
        elif i<5000:#1500:
            y[i,0] = rewards_small_question[i] *0.4
            y[i,1] = rewards_big_question[i] *0.4
        elif i<6000:#1800:
            y[i,0] = rewards_small_question[i] *0.5
            y[i,1] = rewards_big_question[i] *0.5
        elif i<7000:
            y[i,0] = rewards_small_question[i] *0.6
            y[i,1] = rewards_big_question[i] *0.6
        elif i<8000:
            y[i,0] = rewards_small_question[i] *0.7
            y[i,1] = rewards_big_question[i] *0.7
        elif i<9000:
            y[i,0] = rewards_small_question[i] *0.8
            y[i,1] = rewards_big_question[i] *0.8
        elif i<10000:
            y[i,0] = rewards_small_question[i] *0.9
            y[i,1] = rewards_big_question[i] *0.9
        else:
            y[i,0] = rewards_small_question[i]
            y[i,1] = rewards_big_question[i] 
...
def store_cumulative_rewards(all_rewards,all_actions,seed,new_ideal_policy1,new_ideal_policy2,ideal_policy):
    ...
    filename = 'outputs/QLen/QLen_with_cost_gradual_finetuning_train_data_all_rewards_{}.npy'.format(seed)
    np.save(filename,all_rewards)
    filename = 'outputs/QLen/QLen_with_cost_gradual_finetuning_train_data_all_actions_{}.npy'.format(seed)
    np.save(filename,all_actions) 
   
    
    filename = 'outputs/QLen/QLen_with_cost_gradual_finetuning_train_data_new_ideal_policy1_{}.npy'.format(seed)
    np.save(filename,new_ideal_policy1)
    
    filename = 'outputs/QLen/QLen_with_cost_gradual_finetuning_train_data_new_ideal_policy2_{}.npy'.format(seed)
    np.save(filename,new_ideal_policy2)
    
    filename = 'outputs/QLen/QLen_with_cost_gradual_finetuning_train_data_ideal_policy_{}.npy'.format(seed)
...
for seed_num,seed in tqdm(enumerate(lst_seeds)):
    ...
    numQueries = 20000
    dim_context = 768;
    ideal_policy = np.load('outputs/labels_of_coqa_question_length_train_data.npy')
